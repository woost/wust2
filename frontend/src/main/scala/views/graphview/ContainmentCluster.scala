 ...
 class ContainmentCluster(val parent: SimPost, val children: IndexedSeq[SimPost], val depth: Int) {
   ...
   private def regenerateCircleSamples(): Unit = {
     var i = 0
     val n = postCount * sn
     while (i < n) {
       val a = (i % sn) * step
 ...
 object ContainmentClusterSelection extends DataSelection[ContainmentCluster] {
   ...
   override def enterAppend(hull: Selection[ContainmentCluster]): Unit = {
     hull
   ...
   override def draw(hull: Selection[ContainmentCluster]): Unit = {
     hull
