...
-- no membership exists for P1, therefore not allowed to see anything
SELECT cmp_ok(readable_nodes(touuid('A1'), array[touuid('B1')]), '=', array[]::uuid[]);

-- membership exists for P2, therefore allowed to see P1
SELECT cmp_ok(readable_nodes(touuid('A2'), array[touuid('B1')]), '=', array[touuid('B1')]::uuid[]);
