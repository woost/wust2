CREATE FUNCTION views_to_node_schema(IN views jsonb[]) RETURNS jsonb AS $$
    select jsonb_build_object('views', array_to_json(array_agg(result.viewobjects)), 'entities', jsonb_build_object()) as viewlist
    from (select jsonb_build_object('view',unnest(views),'config','{}'::jsonb) as viewobjects) as result;
$$ LANGUAGE 'sql' IMMUTABLE;

ALTER TABLE node
    RENAME column views TO schema;

ALTER TABLE node
    alter column schema type jsonb using views_to_node_schema(schema);

update node set schema = '{"views": null, "entities": {}}' where schema is null;
alter table node alter column schema set not null;

drop function views_to_node_schema;
