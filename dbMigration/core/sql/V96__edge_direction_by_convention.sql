...
with edgeconvention as (
  select * from edge where data->>'type' = any(array['Assigned', 'Author', 'Expanded', 'Invite', 'Member', 'Pinned'])
)
update edge
set
    sourceid = edgeconvention.targetid,
    targetid = edgeconvention.sourceid
from edgeconvention
where edge.sourceid = edgeconvention.sourceid and edge.targetid = edgeconvention.targetid and edge.data->>'type' = edgeconvention.data->>'type';

-- update parent -> child and remove inconsistency
with parentconversion as (
  select * from edge where data->>'type'='Parent'
)
update edge
set
    sourceid = parentconversion.targetid,
    data = case
            when edge.data?'ordering' and edge.data?'deletedAt' then
                jsonb_build_object('type', 'Child', 'deletedAt', parentconversion.data->>'deletedAt', 'ordering', parentconversion.data->>'ordering')
            when edge.data?'ordering' and not(edge.data?'deletedAt') then
                jsonb_build_object('type', 'Child', 'deletedAt', null, 'ordering', parentconversion.data->>'ordering')
            when not(edge.data?'ordering') and edge.data?'deletedAt' then
                jsonb_build_object('type', 'Child', 'deletedAt', parentconversion.data->>'deletedAt', 'ordering', null)
            else
                jsonb_build_object('type', 'Child', 'deletedAt', null, 'ordering', null)
        end,
    targetid = parentconversion.sourceid
...
-- begin
--     case edgetype
--         when 'Child' then return true;
--         when 'LabeledProperty' then return true;
--         when 'Automated' then return true;
--         when 'DerivedFromTemplate' then return true;
--         else return false;
--     end;
-- end;
-- $$ language plpgsql;
...
drop view pinned;
drop view automated;
drop view implements;

create view assigned as select sourceid as source_nodeid, targetid as target_userid, data from edge where data->>'type' = 'Assigned';
create view author as select sourceid as source_nodeid, targetid as target_userid, data from edge where data->>'type' = 'Author';
create view expanded as select sourceid as source_nodeid, targetid as target_userid, data from edge where data->>'type' = 'Expanded';
create view invite as select sourceid as source_nodeid, targetid as target_userid, data from edge where data->>'type' = 'Invite';
create view member as select sourceid as source_nodeid, targetid as target_userid, data from edge where data->>'type' = 'Member';
create view notify as select sourceid as source_nodeid, targetid as target_userid, data from edge where data->>'type' = 'Notify';
create view pinned as select sourceid as source_nodeid, targetid as target_userid, data from edge where data->>'type' = 'Pinned';
...
drop function graph_page_nodes; -- drop function graph_traversed_page_nodes;
drop function readable_graph_page_nodes_with_channels;
-- drop function readable_graph_page_nodes_with_channels_with_orphans;
drop function readable_channel_nodes; -- drop function user_quickaccess_nodes;
...
as $$
    ...
    from node
    left outer join edge on edge.sourceid = node.id -- outer join, because we want to keep the nodes which have no outgoing edges
