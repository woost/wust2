...
as $$
    with cpid as (select (data->>'channelNodeId')::uuid as channelnodeid from node where id = userid and data->>'type' = 'User')
        ...
        union
        select sourceid as nodeid from edge,cpid where edge.targetid = cpid.channelnodeid and data->>'type' = 'Parent' and can_access_node(edge.sourceid, userid) -- all user channels (children of channelnodeid)
        union
...
as $$
    with cpid as (select (data->>'channelNodeId')::uuid as channelnodeid from node where id = userid and data->>'type' = 'User')
        select cpid.channelnodeid as nodeid from cpid  -- channel post of user, containing all user channels
        union
        select sourceid as nodeid from edge,cpid where edge.targetid = cpid.channelnodeid and data->>'type' = 'Parent' and can_access_node(edge.sourceid, userid)
        union
        select unnest(graph_page_nodes(parents, children, userid)) as nodeid -- all nodes, specified by page (transitive children + transitive parents)
        union
        select id from node where id not in (select sourceid from edge where data->>'type' = 'Parent') and can_access_node(id, userid)
$$ language sql;
