update node set role = '{"type":"Stage"}' where node.id in (
    select distinct on(node.id) node.id
    from node
    left join parent p on p.source_childid = node.id
    left join node nodeparent on p.target_parentid = nodeparent.id
    left join parent c on c.target_parentid = node.id
    left join node nodechild on c.source_childid = nodechild.id
    where
    nodeparent.role = '{"type":"Task"}' and
    node.role = '{"type":"Task"}' and
    nodechild.role = '{"type":"Task"}'
);
