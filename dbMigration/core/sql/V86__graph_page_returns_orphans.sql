create or replace function readable_channel_nodes(userid uuid) returns setof uuid as $$
    with recursive channels(id) as (
        ...
        select target_nodeid from member where member.source_userid = userid and can_access_node(userid, member.target_nodeid)
        union
