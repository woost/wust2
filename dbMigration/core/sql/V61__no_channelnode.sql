insert into edge(sourceid,data,targetid) (
    select node.id as sourceid,json_build_object('type', 'Pinned') as data,edge.sourceid as targetid from node join edge on edge.targetid = (node.data->>'channelNodeId')::uuid where node.data->>'type' = 'User' and edge.data->>'type' = 'Parent' and edge.data->>'deletedAt' is NULL
);

delete from node where node.id in (
    select (node.data->>'channelNodeId')::uuid from node where node.data->>'type' = 'User'
);
update node set data = data - 'channelNodeId' where node.data->>'type' = 'User';
