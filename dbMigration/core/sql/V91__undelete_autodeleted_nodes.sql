update edge
set data = data || jsonb '{"deletedAt": null}'
from (
select parent.source_childid, parent.target_parentid
    from parent
    join author on author.target_nodeid = parent.source_childid
    group by parent.source_childid, parent.target_parentid, parent.data
    -- automatically deleted = was deleted exactly one week after creation (creation is a few ms later than (deleted-1week))
    --                       = creation - epsilon < deletedAt - 1week < creation
    having parent.data->>'deletedAt' is not null and
        min((author.data->>'timestamp')::bigint) - (1000*120) < (parent.data->>'deletedAt')::bigint - (1000*(3600*24*7)) and
        (parent.data->>'deletedAt')::bigint - (1000*(3600*24*7)) < min((author.data->>'timestamp')::bigint)
) autodeleted
where data->>'type' = 'Parent' and sourceid = autodeleted.source_childid and targetid = autodeleted.target_parentid;
...
create function readable_channel_nodes(userid uuid) returns setof uuid as $$
    with recursive channels(id) as (
        ...
        select target_nodeid from invite where invite.source_userid = userid and can_access_node(userid, invite.target_nodeid)
        union
...
select induced_subgraph(
    array(
        ...
        union
        -- all available properties for all nodes
        select edge.targetid as propertyid from nodes inner join edge on edge.sourceid = nodes.nodeid where edge.data->>'type' = 'LabeledProperty'
    ),
    userid
);
$$ language sql stable strict;
