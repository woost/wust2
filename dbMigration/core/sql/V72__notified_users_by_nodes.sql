...
create or replace function subscriptions_by_nodeid(startids uuid[])
    returns table (
         id integer, userid uuid, endpointUrl text, p256dh text, auth text, initial_node uuid[]
    ) as $$
    select webpushsubscription.id, webpushsubscription.userid, webpushsubscription.endpointUrl, webpushsubscription.p256dh, webpushsubscription.auth, notifications.initial_node
