drop function if exists traverse_children_unsafe;

create or replace function can_access_node(userid uuid, nodeid uuid) returns boolean as $$
begin
    return true;
end;
$$ language plpgsql STABLE;

select create_traversal_function('traverse_children_unsafe', 'edge', 'targetid', 'Parent', 'sourceid', 10000, 'can_access_node');


create temporary table visited (id uuid NOT NULL);
create unique index on visited (id);

update node set role = '{"type": "Task"}'::jsonb where node.id in (
select unnest(traverse_children_unsafe(array(select targetid from edge where data->>'type' = 'StaticParentIn'), array[]::uuid[], '23c4b272-1d87-2700-000f-32ddc66ed50a'))
);
...
-- drop all temporary functions
drop function traverse_children_unsafe;
