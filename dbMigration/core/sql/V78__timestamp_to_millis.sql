create or replace function millis_to_timestamp(millis anyelement) returns timestamp with time zone as $$
    select to_timestamp(millis::bigint / 1000)
$$ language sql stable strict;

drop function notified_users_search_fast;

create or replace function notified_users_search_fast(startids uuid[], now timestamp default now_utc())
    returns table (
