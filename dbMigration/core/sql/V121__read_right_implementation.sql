    ...
    IF ( not exists(select 1 from node_can_access_mat where node_can_access_mat.nodeid = node_id and node_can_access_mat.complete) ) THEN
        ...
        IF (exists(select 1 from node where id = node_id and (accesslevel IS NULL or accesslevel = 'read' or accesslevel = 'readwrite'))) THEN -- null means inherit for the node, readwrite/public or read/public inherits as well
            ...
            insert into node_can_access_mat (
                select node_id as nodeid, userid, is_complete as complete, max(accesslevel) from (
                    select node_can_access_mat.userid as userid, node_can_access_mat.accesslevel as accesslevel
                    from accessedge
                    ...
                    union all
                    select member.target_userid as userid, (member.data->>'level')::accesslevel as accesslevel
                    from member
                    where member.source_nodeid = node_id
                ) as canaccess
                group by userid
            ) on conflict (nodeid, userid) DO UPDATE set complete = EXCLUDED.complete, accesslevel = EXCLUDED.accesslevel;
        ELSE
            insert into node_can_access_mat (
