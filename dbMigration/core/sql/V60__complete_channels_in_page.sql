...
create function readable_channel_nodes(channelnodeid uuid, userid uuid) returns table(nodeid uuid) as $$
declare
    parents uuid[];
    children uuid[];
begin
    select array_agg(sourceid) into children from edge where edge.targetid = channelnodeid and data->>'type' = 'Parent' and can_access_node(userid, edge.sourceid);
    create temporary table visited (id uuid NOT NULL) on commit drop;
    create unique index on visited (id);
    parents := traverse_parents(children, array[]::uuid[], userid);
    drop table visited;
    return query select distinct unnest(children || parents);
end;
$$ language plpgsql;
...
as $$
    with cpid as (select (data->>'channelNodeId')::uuid as channelnodeid from node where id = userid and data->>'type' = 'User')
        ...
        union
        select cs.nodeid from (select readable_channel_nodes(cpid.channelnodeid, userid) as nodeid from cpid) as cs
        union
