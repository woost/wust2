...
with recursive notified_users(
	...
	-- notification_result.depth
    notification_result.userid
    , array_agg(notification_result.initial_node)
    , notification_result.subscribed_node
	-- , (select new_node.data->>'content' from node as new_node where new_node.id = notification_result.initial_node)
	-- , (select username.data->>'name' from node as username where username.id = notification_result.userid)
	-- , (select subscribed_node_name.data->>'content' from node as subscribed_node_name where subscribed_node_name.id = notification_result.subscribed_node)
	-- , array(select node.data->>'content' from node where node.id = any(notification_result.allowed_members))
	-- , (select current_name.data->>'content' from node as current_name where current_name.id = notification_result.inspected_node)
	-- , array(select node.data->>'content' from node where node.id = any(notification_result.visited))
