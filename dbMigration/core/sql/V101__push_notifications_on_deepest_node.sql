...
with recursive notified_users(
    ...
    , user_finished     -- users for which a notification edge has been found
    ...
    left outer join notify on notify.source_nodeid = child.source_parentid

) select
	depth
	, (select new_node.data->>'content' from node as new_node where new_node.id = notified_users.initial_node)
	, (select username.data->>'name' from node as username where username.id = notified_users.userid)
	, (select subscribed_node_name.data->>'content' from node as subscribed_node_name where subscribed_node_name.id = notified_users.subscribed_node)
	, array(select node.data->>'content' from node where node.id = any(allowed_members))
	, (select current_name.data->>'content' from node as current_name where current_name.id = notified_users.inspected_node)
	, array(select node.data->>'content' from node where node.id = any(visited))
	from notified_users
	-- inner join node as visited_name where visited.id = any(allowed_members)
...
-- select * from notified_users_search_fast(array['244a4541-b46f-c803-3b5d-2a014a191ae6']::uuid[]);
-- outputs ---
select * from node;
select * from notified_users_at_deepest_node(array['244a4541-b46f-c803-3b5d-2a014a191ae6']::uuid[]);
select * from notified_users_at_deepest_node(array['244a4547-0ffb-cc06-3b5d-2a082eaf6e08']::uuid[]);
