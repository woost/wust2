...
create function notified_users_at_subscribed_node(startids uuid[], now timestamp default now_utc())
    returns table (
        userId uuid                 -- user id who will be notified
        , newCreatedNodes uuid[]    -- nodes the user will be notified about
        , subscribedNode uuid       -- node at which the user set to be notified (e.g. channel)
    ) as $$
with recursive notified_users(
    newCreatedNode        -- nodes the user may be notified about
    , userId            -- user id who will be notified
    , subscribedNode   -- node at which the user set to be notified (e.g. channel)
    , allowedUsers   -- all users / members with access to the initial node and hence to be checked against
    , inspectedNode    -- current node that was traversed
    , visited           -- all nodes that have been visited (prevent cycles)
    , depth
) as (
    select
        node.id as newCreatedNode
        ...
        , notify.source_nodeid as subscribedNode
    ...
    select
        notified_users.newCreatedNode as newCreatedNode     -- newCreatedNodes are propageted in each step
        , (case   when node.accesslevel = 'restricted'              --\
                    and notify.target_userid = any(                 ---\
        ...
        , notify.source_nodeid as subscribedNode
        , (case    when node.accesslevel = 'restricted'
						and notified_users.allowedUsers is null
                    then array(
                	...
                	when node.accesslevel = 'restricted'
                    	then array(
							select unnest(notified_users.allowedUsers)
								intersect
							(select target_userid from member
									where member.data->>'level'='readwrite'
									and member.source_nodeid = node.id
                ...
                else notified_users.allowedUsers
        end) as allowedUsers
        , node.id as inspectedNode
        , array_append(notified_users.visited, child.source_parentid) as visited
        , (notified_users.depth + 1) as depth
    from notified_users
    inner join child on child.target_childid = notified_users.inspectedNode
    ...
    , array_agg(notification_result.newCreatedNode)
    , notification_result.subscribedNode
	from notified_users notification_result
    left outer join notified_users notification_filter on notification_result.userId = notification_filter.userId and notification_result.newCreatedNode = notification_filter.newCreatedNode and (notification_result.depth > notification_filter.depth)
    where notification_filter.userId is null and notification_result.userId is not null
        and case  when notification_result.allowedUsers is null
                    then true
                else notification_result.userId = any(notification_result.allowedUsers)
        end
    group by notification_result.userId, notification_result.subscribedNode;
$$ language sql stable;
...
create function subscriptions_by_nodeid(startids uuid[])
    ...
    select webpushsubscription.id, webpushsubscription.userId, webpushsubscription.endpointUrl, webpushsubscription.p256dh, webpushsubscription.auth, notifications.newCreatedNodes as notifiedNodes, notifications.subscribedNode as subscribedNodeId, node.data->>'content' as subscribedNodeContent
        from notify_users_at_subscribed_node(startids) as notifications
        inner join webpushsubscription on webpushsubscription.userId = notifications.userId
        inner join node on node.id = notifications.subscribedNode
$$ language sql stable;
...
drop function notified_users_at_subscribed_node;
