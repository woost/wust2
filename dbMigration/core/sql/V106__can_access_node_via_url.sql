...
begin
    ...
    IF (member_access_level IS NULL) THEN -- if no member edge exists
        ...
        IF (node_access_level IS NULL or node_access_level = 'readwrite') THEN -- null means inherit for the node
            ...
            select bool_or(can_access_node_recursive(userid, contentedge.source_nodeid, visited || nodeid, get_via_url_mode)) into result from contentedge where contentedge.target_nodeid = nodeid;
...
begin
    ...
    return COALESCE(member_access_level = 'readwrite', false); -- if no access level was found => no permission
end;
$$ language plpgsql strict;
