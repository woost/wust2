create or replace function parent_workspaces(nodeid uuid) returns setof uuid as $$
with recursive transitive_parents(id) AS (
    select nodeid
    union -- discards duplicates, therefore handles cycles and diamond cases
    select parent.target_parentid
        FROM transitive_parents
        INNER JOIN parent ON parent.source_childid = transitive_parents.id and parent.data->>'deletedAt' is null
        inner join node currentnode on transitive_parents.id = currentnode.id
        inner join node nextnode on parent.target_parentid = nextnode.id
        where (currentnode.id = nodeid and nextnode.role->>'type' = 'Stage') or
            (currentnode.role->>'type' = 'Stage')
)
select transitive_parents.id from transitive_parents inner join node on transitive_parents.id = node.id
where node.id != nodeid and node.role->>'type' != 'Stage'
$$ language sql stable strict;


insert into edge (select id as sourceid, parent_workspaces(id) as targetid, '{"type": "Parent", "deletedAt": null}'::jsonb as data from (select node.id
from node
left join parent p on p.source_childid = node.id
left join node node_parent on node_parent.id = p.target_parentid
where node.role->>'type' = 'Task' and p.data->>'deletedAt' is null
group by node.id
having array_agg(distinct node_parent.role->>'type') = array['Stage']) as task);


drop function parent_workspaces;
