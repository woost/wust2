ALTER TABLE node
    ADD COLUMN views JSONB[] DEFAULT NULL;
...
as $$
    ...
    array_remove(array_agg(node.views), NULL), array_remove(array_agg(edge.targetid), NULL), array_remove(array_agg(edge.data::text), NULL) -- removing NULL is important (e.g. decoding NULL as a string fails)
    from node
...
select induced_subgraph(
    array(
        ...
        union
        -- all available automated templates for all nodes
        select edge.targetid as templateid from nodes inner join edge on edge.sourceid = nodes.nodeid where edge.data->>'type' = 'Automated'

        union
        -- all available implements nodes for all nodes
        select edge.targetid as templateid from nodes inner join edge on edge.sourceid = nodes.nodeid where edge.data->>'type' = 'Implements'
    ),
    userid
);
$$ language sql stable strict;
