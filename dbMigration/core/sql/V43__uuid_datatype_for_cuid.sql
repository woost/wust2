...
ALTER TABLE node
    alter column id type uuid using cuid_to_uuid(id);

ALTER TABLE edge
    alter column sourceid type uuid using cuid_to_uuid(sourceid),
    alter column targetid type uuid using cuid_to_uuid(targetid),
    add constraint "connection_sourceid_fkey" FOREIGN KEY (sourceid) REFERENCES node(id) ON DELETE CASCADE,
    add constraint "connection_targetid_fkey" FOREIGN KEY (targetid) REFERENCES node(id) ON DELETE CASCADE;

ALTER TABLE password rename column id to userid;

ALTER TABLE password
    alter column userid type uuid using cuid_to_uuid(userid),
    add constraint "fk_user_post" FOREIGN KEY (userid) REFERENCES node(id) ON DELETE CASCADE;
...
select induced_subgraph(
    array(
        with
            cpid as (select (data->>'channelNodeId')::uuid as channelnodeid from node where id = userid and data->>'type' = 'User'),
            nodes as (
            ...
            authors as (select edge.sourceid as authorid from edge, nodes where edge.targetid = nodes.id and (edge.data->>'type' = 'Author' or edge.data->>'type' = 'Member')) -- all available authors for all nodes
...
-- page(parents, children) -> graph as adjacency list
create function graph_page_with_orphans(parents uuid[], children uuid[], userid uuid) returns table(nodeid uuid, data jsonb, joindate timestamp without time zone, joinlevel accesslevel, deleted timestamp without time zone, targetids uuid[], edgeData text[]) as $$
