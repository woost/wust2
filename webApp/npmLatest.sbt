...
val npmDevLatest = taskKey[Unit]("Get latest npm dev-package versions")
npmDevLatest := {
  val logger = streams.value.log
  val projsDeps =
    Def.task {
      (projectID.value.name, (npmDevDependencies in Compile).??(Nil).value)
    }.all(ScopeFilter(inAnyProject)).value
  ...
  for ((proj, deps) <- projsDeps) {
    ...
    val updates = latestVersions.filter(x => x._2 != x._3)
    if (updates.nonEmpty) {
      logger.info(s"$BOLD${pad(proj, maxProjLen)} $RESET${updates.length} new npm dev-package versions")
      for ((pkg, cur, latest) <- updates)
        logger.info(s"  $BOLD${pad(pkg, maxPkgLen)}$RESET ${pad(cur, maxVerLen)} -> $latest")
    }
  }
