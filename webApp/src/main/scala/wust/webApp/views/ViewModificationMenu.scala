...
//    }
//  }
//  private def removeView(currentView: Var[View], done: Observer[Unit], nodeRx: Rx[Option[Node]], view: View): Unit = {
//    done.onNext(())
//    val node = nodeRx.now
//    node.foreach { node =>
//      val currentViews = node.views.getOrElse(Nil)
...
//  //TODO: gets triggered 3 times when adding a view. Should only trigger once
//  private def addNewView(currentView: Var[View], done: Observer[Unit], nodeRx: Rx[Option[Node]], existingViews: Rx[List[View]], newView: View): Unit = {
//    scribe.info(s"ViewModificationMenu.addNewView($newView)")
//    if (View.selectableList.contains(newView)) { // only allow defined views
//      done.onNext(())
//      val node = nodeRx.now
