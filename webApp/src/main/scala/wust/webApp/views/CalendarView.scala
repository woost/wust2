...
object CalendarView {
  ...
  private def eventsToChunks(unsortedEvents: Seq[Event]):CalendarData = {
    ...
    val weekHeight = mutable.HashMap.empty[String,Int].withDefaultValue(minWeekHeight) // how high one week row needs to be
  ...
  def apply(focusState: FocusState)(implicit ctx: Ctx.Owner): VNode = {
    ...
    val calendarData:Rx[CalendarData] = Rx {
      ...
      val parentIdxOpt = graph.idToIdx(focusState.focusedId)
        ...
        graph.childrenIdx.foreachElement(parentIdx) { nodeIdx =>
          ...
          if(node.role == NodeRole.Task) {
            ...
            props.get(EdgeData.LabeledProperty.dueDate.key).foreach { propValues =>
              propValues.headOption.foreach { propValue =>
    ...
    div(
      ...
      div(
        ...
        opacity := 0.4,
        fontWeight.bold,
        List.tabulate(7)(i => div(flex := "1", marginLeft := s"${gridSpacing}px", padding := "5px", weekDays(i), overflow.hidden)),
      ),
      div(
        Styles.flexStatic,
        Styles.flex,
        ...
        Rx {
          List.tabulate(weeksInMonth()){weekOfMonth =>
            ...
            div(
              flex := "1",
              minHeight := s"${(1+calendarData().weekHeight(wKey)) * eventLineHeight + gridSpacing}px",
              Styles.flexStatic,
              Styles.flex,
