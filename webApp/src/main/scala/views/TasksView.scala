...
object TasksView {
  def apply(state: GlobalState)(implicit ctx: Ctx.Owner): VNode = {
    def topLevelStageExists(page:Page, graph:Graph) = {
      page.parentId.fold(false) { parentId =>
        ...
        graph.notDeletedChildrenIdx.exists(parentIdx) { childIdx =>
          ...
          node.role == NodeRole.Stage && !doneNodes.contains(childIdx)
        }
      }
    }
    ...
    state.rawGraph.foreach{ graph =>
      if(lastPage != state.page.now) {
        kanbanSwitch() = topLevelStageExists(state.page.now, graph)
