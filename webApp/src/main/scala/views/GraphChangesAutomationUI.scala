            ...
            div(
              ...
              div(
                ...
                Components.removeableList[Node](
                  templates,
                  state.eventProcessor.changes.redirectMap(templateNode => GraphChanges(delEdges = Set(Edge.Automated(nodeId, templateNode.id)))),
    ...
    val actions: VDomModifier = VDomModifier(
      ...
      button(
        ...
        onClick.mapTo {
          ...
          GraphChanges(addEdges = Set(Edge.Parent(templateNode.id, nodeId), Edge.Automated(nodeId, templateNode.id)), addNodes = Set(templateNode))
      ...
      Components.searchInGraph(state.graph, placeholder = "Add existing template", filter = {
        ...
        state.eventProcessor.changes onNext GraphChanges(addEdges = Set(Edge.Automated(nodeId, selectedTemplateNodeId)))
      },
    )
