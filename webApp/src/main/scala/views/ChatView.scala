...
object ChatView extends View {
  ...
  def chatMessage(state:GlobalState, post: Node, graph:Graph, currentUser: UserInfo)(implicit ctx: Ctx.Owner): VNode = {
    val postTags: Seq[Node] = (graph.ancestors(post.id) diff graph.channelNodeIds.toSeq diff state.page.now.parentIds).map(graph.nodesById(_)) //TODO: is this a performance problem and should be cached?
