    ...
    val deleteItem =
      (bookmarked && canWrite).ifTrue[VDomModifier](div(
        ...
        onClick foreach {
          ...
          Toast(s"Deleted Workspace '${StringOps.trimToMaxLength(channel.str, 10)}'", click = () => state.page() = Page(channel.id), level = ToastLevel.Success)
        }
      ))
    ...
    div(
      ...
      Rx {
        ...
        Seq(
          ...
          MkLabel(currentView, pageStyle, View.Conversation, freeRegular.faComments),
