...
//            embeddedPdf(maxHeight := maxImageHeight, width := "100%")
  ...
  def readObserver(state: GlobalState, nodeId: NodeId)(implicit ctx: Ctx.Owner): VDomModifier = {
    ...
    Rx {
      isUnread() match {
        ...
        case false => unreadChildren() match {
