...
//    dom.window.setTimeout(() => code, timeout = 0)
  ...
  def markdownVNode(str: String) = div.thunkStatic(keyValue(str))(VDomModifier(cls := "markdown", div(innerHTML := UnsafeHTML(markdownString(str))))) // intentionally double wrapped. Because innerHtml does not compose with other modifiers
