...
//  import EdgeComponents._
  ...
  case class Author(nodeId: NodeId, data: EdgeData.Author, userId: UserId) extends NodeUserEdge {
    def copyId(sourceId: NodeId, targetId: NodeId) = copy(nodeId = sourceId, userId = UserId(targetId))
  }

  case class Expanded(nodeId: NodeId, userId: UserId) extends NodeUserEdge {
    def data = EdgeData.Expanded
    def copyId(sourceId: NodeId, targetId: NodeId) = copy(nodeId = sourceId, userId = UserId(targetId))
  }

  case class Invite(nodeId: NodeId, userId: UserId) extends NodeUserEdge {
    def data = EdgeData.Invite
    def copyId(sourceId: NodeId, targetId: NodeId) = copy(nodeId = sourceId, userId = UserId(targetId))
  }

  case class Member(nodeId: NodeId, data: EdgeData.Member, userId: UserId) extends NodeUserEdge {
    def level = data.level
    def copyId(sourceId: NodeId, targetId: NodeId) = copy(nodeId = sourceId, userId = UserId(targetId))
  }
  case class Notify(nodeId: NodeId, userId: UserId) extends NodeUserEdge {
    def data = EdgeData.Notify
    def copyId(sourceId: NodeId, targetId: NodeId) = copy(nodeId = sourceId, userId = UserId(targetId))
  }

  case class Pinned(nodeId: NodeId, userId: UserId) extends NodeUserEdge {
    def data = EdgeData.Pinned
    def copyId(sourceId: NodeId, targetId: NodeId) = copy(nodeId = sourceId, userId = UserId(targetId))
  }

  // Content-Edges
  case class Automated(nodeId: NodeId, templateNodeId: TemplateId) extends ContentEdge {
    def sourceId = nodeId
  ...
  case class Child(parentId: ParentId, data: EdgeData.Child, childId: ChildId) extends ContentEdge {
  ...
  case class DerivedFromTemplate(nodeId: NodeId, data: EdgeData.DerivedFromTemplate, referenceNodeId: TemplateId) extends ContentEdge {
    def sourceId = nodeId
  ...
  case class LabeledProperty(nodeId: NodeId, data: EdgeData.LabeledProperty, propertyId: PropertyId) extends ContentEdge {
    def sourceId = nodeId
