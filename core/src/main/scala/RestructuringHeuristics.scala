...
case object PostHeuristic {
  ...
  def apply(graph: Graph, heuristic: NlpHeuristic, posts: List[NodeId], num: Option[Int]): List[ApiResult] = {
    heuristic match {
  ...
  private[this] def wrapHeuristic(f: List[Node.Content] => List[Result], posts: List[Node.Content], num: Option[Int]): List[Result] = {
  ...
  def hamming(graph: Graph, posts: List[NodeId], num: Option[Int]): List[ApiResult] = {
   ...
   def jaccard(graph: Graph, posts: List[NodeId], num: Option[Int], nGramValue: Int = 3): List[ApiResult] = {
   ...
   def nGram(graph: Graph, posts: List[NodeId], num: Option[Int], nGramValue: Int = 3): List[ApiResult] = {
     wrapStringMetric(NGramMetric(nGramValue), graph, posts, num)
   }

   def overlap(graph: Graph, posts: List[NodeId], num: Option[Int], nGramValue: Int = 3): List[ApiResult] = {
   ...
   def weightedLevenshtein(graph: Graph, posts: List[NodeId], num: Option[Int], delWeight: Int, insWeight: Int, subWeight: Int): List[ApiResult] = {
