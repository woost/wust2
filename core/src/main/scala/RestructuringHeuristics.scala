...
case object PostHeuristic {
  ...
  implicit def IntToDoubleMetric(m: StringMetric[Int]): StringMetric[Double] = new StringMetric[Double] {
    override def compare(a: Array[Char], b: Array[Char]): Option[Double] = m.compare(a, b).map(_.toDouble)
    override def compare(a: String, b: String): Option[Double] = m.compare(a, b).map(_.toDouble)
  }

  def apply(graph: Graph, heuristic: NlpHeuristic, posts: List[PostId], num: Option[Int]): List[ApiResult] = {
    heuristic match {
      ...
      case NlpHeuristic.WeightedLevenshtein(delWeight, insWeight, subWeight) => weightedLevenshtein(graph, posts, num, delWeight, insWeight, subWeight)
      case _ => List(Heuristic.IdResult(None, posts))
    }
  }

  private[this] def wrapHeuristic(f: List[Post] => List[Result], posts: List[Post], num: Option[Int]): List[Result] = {
  ...
  private[this] def wrapStringMetric(metric: StringMetric[Double], graph: Graph, postIds: List[PostId], num: Option[Int]): List[ApiResult] = {
    val posts = postIds.map(graph.postsById)
    def f(p: List[Post]): List[Result] = (for {
        l <- p.combinations(2).toList
        } yield {
          Heuristic.PostResult(metric.compare(l.head.content.str, l(1).content.str), l)
        }).sortBy(_.measure)
   ...
   def diceSorensen(graph: Graph, posts: List[PostId], num: Option[Int], nGramValue: Int = 3): List[ApiResult] = {
     wrapStringMetric(DiceSorensenMetric(nGramValue), graph, posts, num)
   }

  def hamming(graph: Graph, posts: List[PostId], num: Option[Int]): List[ApiResult] = {
    wrapStringMetric(HammingMetric, graph, posts, num)
  }

   def jaccard(graph: Graph, posts: List[PostId], num: Option[Int], nGramValue: Int = 3): List[ApiResult] = {
     wrapStringMetric(JaccardMetric(nGramValue), graph, posts, num)
   }

   def jaro(graph: Graph, posts: List[PostId], num: Option[Int]): List[ApiResult] = {
     wrapStringMetric(JaroMetric, graph, posts, num)
   }

   def jaroWinkler(graph: Graph, posts: List[PostId], num: Option[Int]): List[ApiResult] = {
     wrapStringMetric(JaroWinklerMetric, graph, posts, num)
   }

   def levenshtein(graph: Graph, posts: List[PostId], num: Option[Int]): List[ApiResult] = {
     wrapStringMetric(LevenshteinMetric, graph, posts, num)
   }

   def nGram(graph: Graph, posts: List[PostId], num: Option[Int], nGramValue: Int = 3): List[ApiResult] = {
     wrapStringMetric(NGramMetric(nGramValue), graph, posts, num)
   }

   def overlap(graph: Graph, posts: List[PostId], num: Option[Int], nGramValue: Int = 3): List[ApiResult] = {
     wrapStringMetric(OverlapMetric(nGramValue), graph, posts, num)
   }

   def ratcliffObershelp(graph: Graph, posts: List[PostId], num: Option[Int]): List[ApiResult] = {
     wrapStringMetric(RatcliffObershelpMetric, graph, posts, num)
   }

   def weightedLevenshtein(graph: Graph, posts: List[PostId], num: Option[Int], delWeight: Int, insWeight: Int, subWeight: Int): List[ApiResult] = {
