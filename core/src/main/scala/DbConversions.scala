...
object DbConversions {
  ...
  implicit def forClient(post: Data.Post) = Post(post.id, post.content, post.author, post.created, post.modified)
  ...
  implicit def forDb(post: Post) = Data.Post(post.id, post.content, post.author, post.created, post.modified)
  ...
  implicit def forDbUsers(users: Set[User.Persisted]) = users.map(forDb _)
