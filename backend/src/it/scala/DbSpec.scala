...
class DbSpec extends AsyncFreeSpec with MustMatchers {
  "post" - {
    "add" in Db.post("t").map { post =>
      post.title mustEqual "t"
    }

    "get" in Db.post("t").flatMap { post =>
    ...
    "update" in Db.post("t").flatMap { post =>
    ...
    "delete" in Db.post("t").flatMap { post =>
  ...
  "connects" - {
    val post = Await.result(Db.post("t"), 10.seconds)

    "newPost" in Db.connects.newPost("nu", post.id).map { case (newPost, connects) =>
      newPost.title mustEqual "nu"
      connects.sourceId mustEqual newPost.id
      connects.targetId mustEqual post.id
    }

    "add" in Db.post("nu").flatMap { newPost =>
    ...
    "add hyper" in Db.connects.newPost("nu", post.id).flatMap { case (newPost, connects) =>
    ...
    "delete" in Db.connects.newPost("nu", post.id).flatMap { case (_, connects) =>
  ...
  "contains" - {
    ...
    "delete" in Db.post("nu").flatMap { newPost =>
